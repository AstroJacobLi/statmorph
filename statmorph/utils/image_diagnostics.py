...
def make_figure(morph, asinh_a=0.05, framealpha=0.8):
    """
    Creates a figure analogous to Fig. 4 from Rodriguez-Gomez et al. (2019)
    for a given ``SourceMorphology`` object. Using ASinh mapping.
...
#         raise TypeError('Input must be of type SourceMorphology.')
    ...
    # Original image #
    ##################
    ax = _get_ax(fig, 0, 0, nrows, ncols, wpanel,
                 hpanel, htop, eps, wfig, hfig)
    ...
    # Sersic fit #
    ##############
    ax = _get_ax(fig, 0, 1, nrows, ncols, wpanel,
                 hpanel, htop, eps, wfig, hfig)
    ...
    # Sersic residual #
    ###################
    ax = _get_ax(fig, 0, 2, nrows, ncols, wpanel,
                 hpanel, htop, eps, wfig, hfig)
    y, x = np.mgrid[0:ny, 0:nx]
    ...
    # Asymmetry residual #
    ######################
    ax = _get_ax(fig, 0, 3, nrows, ncols, wpanel,
                 hpanel, htop, eps, wfig, hfig)
    ...
    # Original segmap #
    ###################
    ax = _get_ax(fig, 1, 0, nrows, ncols, wpanel,
                 hpanel, htop, eps, wfig, hfig)
    ax.imshow(image, cmap='gray', origin='lower',
              norm=simple_norm(image, stretch='asinh', asinh_a=asinh_a))
    ...
    # Gini segmap #
    ###############
    ax = _get_ax(fig, 1, 1, nrows, ncols, wpanel,
                 hpanel, htop, eps, wfig, hfig)
    ax.imshow(image, cmap='gray', origin='lower',
              norm=simple_norm(image, stretch='asinh', asinh_a=asinh_a))
    # Show Gini segmap
    ...
    ax.contour(Z, contour_levels, colors=contour_colors, linewidths=1.5)
    # Some text
    text = r'$\left\langle {\rm S/N} \right\rangle = %.4f$' % (
        morph.sn_per_pixel,)
    ...
    # Watershed segmap #
    ####################
    ax = _get_ax(fig, 1, 2, nrows, ncols, wpanel,
                 hpanel, htop, eps, wfig, hfig)
    ...
    ##########################
    ax = _get_ax(fig, 1, 3, nrows, ncols, wpanel,
                 hpanel, htop, eps, wfig, hfig)
